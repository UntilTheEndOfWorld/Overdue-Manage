<template>
  <div class="app-container" v-if="show">
    ...
    <el-table v-loading="loading" :data="omsOrderList" border @selection-change="handleSelectionChange"
      ...
      <el-table-column label="订单状态" prop="status" width="200">
        <template v-slot="scope">
          <div>
          ...
          <!-- 快递配送 -->
          <div v-if="scope.row.deliverySn && (!scope.row.deliveryMethod || scope.row.deliveryMethod === 'express')">
            物流单号：{{ scope.row.deliverySn }}
            <el-link @click="copy(scope.row.deliverySn)" :underline="false">
              <i class="el-icon-document-copy el-icon--right"></i>
            </el-link>
          </div>
          <!-- 到店取货显示取件码 -->
          <div v-if="scope.row.deliveryMethod === 'pickup' && scope.row.pickupCode && scope.row.status >= 1">
            <div style="color: #409EFF; font-weight: bold;">
              取件码：{{ scope.row.pickupCode }}
              <el-link @click="copy(scope.row.pickupCode)" :underline="false">
                <i class="el-icon-document-copy el-icon--right"></i>
              </el-link>
            </div>
          </div>
          <!-- 同城配送显示同城配送号码 -->
          <div v-if="scope.row.deliveryMethod === 'local' && scope.row.localDeliveryCode && scope.row.status >= 1">
            <div style="color: #67C23A; font-weight: bold;">
              同城配送号：{{ scope.row.localDeliveryCode }}
              <el-link @click="copy(scope.row.localDeliveryCode)" :underline="false">
                <i class="el-icon-document-copy el-icon--right"></i>
              </el-link>
            </div>
          </div>
      ...
      <el-table-column label="订单编号/操作" class-name="small-padding fixed-width" width="280" fixed="right" >
        <template slot-scope="scope" >
          <div style="float: right">
            {{ scope.row.orderSn }}
            <el-link @click="copy(scope.row.orderSn)" :underline="false">
              <i class="el-icon-document-copy el-icon--right"></i>
            </el-link>
          </div>
          <div style="float: right">
          ...
          <!-- 到店取货查看取件码按钮 -->
          <div v-if="scope.row.deliveryMethod === 'pickup' && scope.row.pickupCode && scope.row.status >= 1" style="float: right; margin-top: 5px;">
            <el-button
              size="mini"
              type="primary"
              icon="el-icon-view"
          ...
          <!-- 同城配送查看配送号按钮 -->
          <div v-if="scope.row.deliveryMethod === 'local' && scope.row.localDeliveryCode && scope.row.status >= 1" style="float: right; margin-top: 5px;">
            <el-button
              size="mini"
              type="success"
              icon="el-icon-view"
              @click="showLocalDeliveryCode(scope.row)"
            >查看同城配送号
            </el-button>
          </div>
        </template>
      ...
      <el-form ref="deliveryForm" :model="deliveryObj.form" :rules="deliveryObj.rules" label-width="100px">
        <el-form-item label="快递公司" prop="expressName">
          <el-select v-model="deliveryObj.form.expressName" placeholder="请选择快递公司" clearable size="small"
            ...
            <el-option label="圆通快递" value="3"/>
          </el-select>
        </el-form-item>
        <el-form-item label="快递单号" prop="expressSn" v-if="deliveryObj.form.expressName !== 'local'">
    ...
    <!-- 取消发货确认对话框 -->
    <el-dialog title="取消发货确认" :visible.sync="cancelDeliveryObj.open" width="400px" append-to-body>
      <div style="text-align: center; padding: 20px 0;">
        <i class="el-icon-warning" style="font-size: 48px; color: #E6A23C; margin-bottom: 20px;"></i>
        <p style="font-size: 16px; margin-bottom: 10px;">确认要取消发货吗？</p>
        <p style="color: #909399; font-size: 14px;">取消后订单状态将退回待发货，发货信息将被清空</p>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button @click="cancelDeliveryObj.open = false">取 消</el-button>
        <el-button type="danger" @click="confirmCancelDelivery">确认取消</el-button>
      </div>
    </el-dialog>
...
export default {
    ...
    return {
      ...
      deliveryObj: {
        ...
        rules: {
          ...
          expressSn: [
            {required: true, message: "快递单号不能为空", trigger: "blur", validator: (rule, value, callback) => {
              if (this.deliveryObj.form.expressName === 'local') {
                callback(); // 同城配送不需要快递单号
              } else if (!value) {
  ...
  methods: {
    ...
    submitDelivery() {
      this.$refs['deliveryForm'].validate((valid) => {
        if (valid) {
          // 如果是同城配送，生成配送号
          if (this.deliveryObj.form.expressName === 'local') {
            this.deliveryObj.form.expressSn = 'LD' + Date.now(); // 生成同城配送号
          }
    ...
    handleCancelDelivery(row) {
      this.cancelDeliveryObj.orderId = row.id
      this.cancelDeliveryObj.orderRow = row
      this.cancelDeliveryObj.open = true
    },
    // 确认取消发货
    confirmCancelDelivery() {
      // 这里需要调用后端API来取消发货
      // 由于没有现成的API，我们先模拟前端状态更新
      this.$modal.confirm('确认要取消发货吗？此操作不可逆。').then(() => {
        // 模拟API调用成功
        this.$modal.msgSuccess('取消发货成功')
        this.cancelDeliveryObj.open = false
        
        // 更新本地订单状态
        const order = this.omsOrderList.find(item => item.id === this.cancelDeliveryObj.orderId)
        if (order) {
          order.status = 1 // 退回待发货状态
          order.deliveryTime = null // 清空发货时间
          order.deliverySn = null // 清空快递单号
          order.deliveryCompany = null // 清空快递公司
          // 清空同城配送相关字段
          if (order.localDeliveryCode) {
            order.localDeliveryCode = null
          }
          if (order.deliveryMethod === 'local') {
            order.deliveryMethod = null
          }
          // 清空其他可能的发货相关字段
          if (order.expressName) {
            order.expressName = null
          }
          if (order.expressSn) {
            order.expressSn = null
          }
          if (order.pickupCode) {
        ...
        // 清空取消发货对象
        this.cancelDeliveryObj.orderId = null
        this.cancelDeliveryObj.orderRow = null
      }).catch(() => {
        // 用户取消操作
      })
    },
    ...
    showPickupCode(row) {
      this.$modal.alert(
        `取件码：${row.pickupCode}\n\n请到店出示此码取货\n\n门店地址：靖西市新靖镇凤凰路1254号绿城美丽园6号楼地下一层\n营业时间：09:00-22:00`,
        '到店取货取件码',
        {
          confirmButtonText: '复制取件码',
          callback: () => {
            this.copy(row.pickupCode)
          }
        }
      )
    },
    // 查看同城配送号码
    showLocalDeliveryCode(row) {
      this.$modal.alert(
        `同城配送号：${row.localDeliveryCode}\n\n该订单将通过同城配送送达\n配送范围：靖西市城区\n预计配送时间：2-4小时\n\n请保持手机畅通，配送员将会联系您。`,
        '同城配送信息',
        {
          confirmButtonText: '复制配送号',
          callback: () => {
            this.copy(row.localDeliveryCode)
          }
        }
      )
    }
