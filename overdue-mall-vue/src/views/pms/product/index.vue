    ...
    <div v-show="show">
      <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="100px" size="medium" class="ry_form">
        ...
        <el-form-item label="是否轮播" prop="isBanner">
          <DictRadio v-model="queryParams.isBanner" @change="handleQuery" size="small"
                     :radioData="dict.type.pms_is_banner" :showAll="'all'"/>
        </el-form-item>
      ...
      <el-table v-loading="loading" :data="pmsProductList" @selection-change="handleSelectionChange" border>
        ...
        <el-table-column label="轮播状态" prop="isBanner">
          <template slot-scope="{ row }">
            <dict-tag :value="row.isBanner" prop-name="pms_is_banner"/>
          </template>
        </el-table-column>
        <el-table-column label="轮播标题" prop="bannerTitle" min-width="120">
          <template slot-scope="{ row }">
            <span v-if="row.bannerTitle">{{ row.bannerTitle }}</span>
...
export default {
  ...
  dicts: ['pms_publish_status', 'pms_is_banner'],
  data() {
    return {
      show: true,
      // 遮罩层
