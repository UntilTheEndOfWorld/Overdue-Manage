...
import com.overdue.common.utils.ip.AddressUtils;
import com.overdue.common.utils.ip.IpUtils;
import com.overdue.common.utils.uuid.IdUtils;
...
public class TokenService {
    ...
    /**
     * 根据token获取用户身份信息（用于WebSocket等场景）
     *
     * @param token 令牌
     * @return 用户信息
     */
    ...
    public LoginMember getLoginMember(HttpServletRequest request) {
        // 获取请求携带的令牌
        String token = getToken(request);
        return getLoginMemberByToken(token);
    }
    ...
    public LoginMember getLoginMemberByToken(String token) {
        if (StringUtils.isNotEmpty(token)) {
            try {
                ...
                Object cachedObject = redisCache.getCacheObject(userKey);

                LoginMember loginMember = null;
                if (cachedObject instanceof LoginMember) {
                    loginMember = (LoginMember) cachedObject;
                } else if (cachedObject instanceof com.alibaba.fastjson2.JSONObject) {
                    // 如果是JSONObject，需要转换为LoginMember
                    com.alibaba.fastjson2.JSONObject jsonObject = (com.alibaba.fastjson2.JSONObject) cachedObject;
                    loginMember = jsonObject.toJavaObject(LoginMember.class);
                }

                return loginMember;
            } catch (Exception e) {
                System.out.println("TokenService.getLoginMemberByToken: 解析token时发生异常: " + e.getMessage());
                e.printStackTrace();
            }
        }
        return null;
    ...
    public void verifyMemberToken(LoginMember loginUser) {
        long expireTime = loginUser.getExpireTime();
        long currentTime = System.currentTimeMillis();
        // 滑动过期：每次请求都刷新token，确保7天有效期
        if (expireTime - currentTime > 0) {
            // 如果token还未过期，则刷新token有效期
