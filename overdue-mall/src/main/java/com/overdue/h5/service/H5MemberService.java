...
import com.overdue.h5.domain.form.BindOpenIDForm;
import com.overdue.h5.domain.form.H5AccountLoginForm;
import com.overdue.h5.domain.form.H5SmsLoginForm;
...
import com.overdue.manager.ums.mapper.MemberLogininforMapper;
import com.overdue.manager.ums.mapper.MemberMapper;
import com.overdue.manager.ums.mapper.MemberWechatMapper;
import com.overdue.manager.ums.service.MemberLogininforService;
import com.overdue.wechat.WechatAuthService;
...
import com.overdue.common.utils.*;
...
public class H5MemberService {
    ...
    public H5LoginVO smsLogin(String data) {
        ...
        if (member == null) {
            // 新会员，注册并登录
            member = new Member();
            member.setPhone(request.getMobile()); // 保存完整手机号
            member.setPhoneEncrypted(AesCryptoUtils.encrypt(aesKey, request.getMobile()));
            member.setPhoneHidden(PhoneUtils.hidePhone(request.getMobile()));
            member.setNickname("用户" + request.getMobile().substring(7, 11));
            member.setStatus(Constants.MEMBER_ACCOUNT_STATUS.NORMAL);
    ...
    private Member createOrUpdateMember(String openId, String mobile) {
        ...
        if (member == null) {
            // 新会员，注册并登录
            member = new Member();
            member.setPhone(mobile); // 保存完整手机号
    ...
    public boolean updateAvatar(String avatarUrl) {
        ...
        int rows = memberMapper.update(null, wrapper);
        return rows > 0;
    }
}
