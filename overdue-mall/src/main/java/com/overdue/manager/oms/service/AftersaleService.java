...
import com.overdue.manager.oms.convert.OrderOperateHistoryConvert;
...
import com.overdue.manager.oms.domain.vo.*;
...
public class AftersaleService {
    ...
    public List<ManagerRefundOrderVO> selectList(ManagerAftersaleOrderForm query, Pageable page) {
        ...
        Set<Long> idSet = managerRefundOrderVOS.stream().map(ManagerRefundOrderVO::getOrderId)
                .collect(Collectors.toSet());
        // 查一下orderSn集合
        QueryWrapper<Order> orderQw = new QueryWrapper<>();
        orderQw.in("id", idSet);
        Map<Long, Order> orderMap = orderMapper.selectList(orderQw).stream()
                .collect(Collectors.toMap(Order::getId, it -> it, (v1, v2) -> v2));
        // 封装售后单商品数据
        QueryWrapper<OrderItem> orderItemQw = new QueryWrapper<>();
    ...
    public void dealWith(DealWithAftersaleForm request, Long userId, String optUserName) {
        ...
        // 售后状态与售后类型是否对应
        if (Constants.OptType.AGREE.equals(request.getOptType())
                || Constants.OptType.REFUSE.equals(request.getOptType())) {
        ...
        // 是否需要发起退款
        if ((request.getOptType() == Constants.OptType.GIVING
                || (request.getOptType() == Constants.OptType.AGREE && aftersale.getType() == 1))) {
    ...
    public void tradeRefund(Aftersale returnApply, Order order, LocalDateTime optDate, Long userId) {
        ...
        // history.getMoney().multiply(new BigDecimal("100")).longValue(),
        // returnApply.getReason());
        // log.info("发起微信退款返回信息,tradeRefund:{}", JSONObject.toJSONString(wechatResponse
        // == null ? "" : wechatResponse));
        //
        // if (wechatResponse != null
        // && Arrays.asList("PROCESSING",
        // "SUCCESS").contains(wechatResponse.getStatus().name())) {
        // qw = new QueryWrapper<>();
        // qw.eq("order_id", order.getId()).eq("op_type", 3);
        // WechatPaymentHistory refundHistory =
        // wechatPaymentHistoryMapper.selectOne(qw);
        ...
        // LambdaQueryWrapper<WechatPaymentHistory> queryWrapper =
        // Wrappers.lambdaQuery();
        // queryWrapper.eq(WechatPaymentHistory::getOrderId, order.getPayId());
        // queryWrapper.eq(WechatPaymentHistory::getOpType,
        // Constants.PaymentOpType.PAY);
        // WechatPaymentHistory payHistory =
        // wechatPaymentHistoryMapper.selectOne(queryWrapper);
        ...
        // wechatPaymentHistory.setMoney(returnApply.getReturnAmount().multiply(new
        // BigDecimal("100")));
    ...
    public void refundOrderExc(RefundNotification weChatRefundNotify) {
        ...
        if ("SUCCESS".equals(weChatRefundNotify.getRefundStatus().name())) {
            ...
            OrderItem orderItem = orderItemMapper
                    .selectOne(new QueryWrapper<OrderItem>().eq("order_id", order.getId()));
