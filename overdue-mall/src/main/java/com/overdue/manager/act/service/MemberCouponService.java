...
import com.overdue.manager.act.domain.query.MemberCouponQuery;
...
public class MemberCouponService {
    ...
    public Page<MemberCouponVO> selectList(MemberCouponQuery query, Pageable page) {
        ...
        Map<Long, Member> memberMap = memberMapper.selectList(new QueryWrapper<Member>().in("id", memberIds))
                .stream().collect(Collectors.toMap(it -> it.getId(), it -> it, (v1, v2) -> v2));
    ...
    public Boolean judgeCouponCanUse(MemberCoupon item, Collection<Product> products) {
        ...
        if (!Objects.equals(1, item.getUseScope())) {
            List<Long> couponProducts = Arrays.stream(item.getProductIds().split(",")).map(it -> Long.parseLong(it))
                    .collect(Collectors.toList());
            if (Objects.equals(2, item.getUseScope())
                    && products.stream().noneMatch(it -> couponProducts.contains(it.getId()))) {
                // 指定商品
                return false;
            }
            if (Objects.equals(3, item.getUseScope())
                    && products.stream().anyMatch(it -> couponProducts.contains(it.getId()))) {
                // 指定商品不包括
                return false;
            }
        }
    ...
    private BigDecimal calcMinAmount(Collection<Product> products, MemberCoupon coupon) {
        ...
        if (!Objects.equals(1, coupon.getUseScope())) {
            ids = Arrays.stream(coupon.getProductIds().split(",")).map(it -> Long.parseLong(it))
                    .collect(Collectors.toList());
        } else {
            ids = new ArrayList<>();
        }
        switch (coupon.getUseScope()) {
            ...
            case 2:
                return products.stream().filter(it -> ids.contains(it.getId())).map(Product::getPrice)
