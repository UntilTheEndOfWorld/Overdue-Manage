package com.overdue.manager.act.service;
...
import com.overdue.manager.ums.domain.entity.MemberAccount;
import com.overdue.manager.ums.mapper.MemberAccountMapper;
...
import com.overdue.manager.act.domain.query.CouponActivityQuery;
...
public class CouponActivityService {
    ...
    public Page<CouponActivityVO> selectList(CouponActivityQuery query, Pageable page) {
        ...
        // 查找已使用张数
        Map<Long, Integer> useCountMap = memberCouponMapper
                .countUseCoupon((list.stream().map(it -> it.getId()).collect(Collectors.toSet())))
                .stream().collect(Collectors.toMap(it -> it.getId(), it -> it.getUseCount(), (v1, v2) -> v2));
        Set<Long> productIds = new HashSet<>();
        list.stream().filter(
                it -> Arrays.asList(2, 3).contains(it.getUseScope()) && StringUtils.isNotEmpty(it.getProductIds()))
                .forEach(it -> {
                    productIds.addAll(Arrays.stream(it.getProductIds().split(",")).map(item -> Long.parseLong(item))
        ...
        for (CouponActivity couponActivity : list) {
            ...
            if (Arrays.asList(2, 3).contains(couponActivity.getUseScope())
                    && StringUtils.isNotEmpty(couponActivity.getProductIds())) {
    ...
    public Page<CouponActivityVO> selectListByH5(Pageable page) {
        ...
        Map<Long, Integer> countMap = memberCouponMapper.countGetCoupon(ids, SecurityUtil.getLocalMember().getId())
                .stream().collect(Collectors.toMap(it -> it.getId(), it -> it.getGetCount(), (v1, v2) -> v2));
        List<CouponActivityVO> resList = new ArrayList<>();
